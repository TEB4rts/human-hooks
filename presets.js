import { actionMatches, any, amountAbove, confidenceBelow, externalDomainNotAllowed, flag, policy, recipientCountAbove, tagIncludes, } from './policies.js';
export function createUniversalValidationPolicies(options = {}) {
    ...
    return [
        ...
        policy({
            ...
            when: any(tagIncludes(destructiveTag), actionMatches(/delete|destroy|terminate|wipe|revoke|drop|shutdown|remove|disable/i)),
        }),
        policy({
        ...
        policy({
            ...
            when: any(tagIncludes(privilegedTag), actionMatches(/grant|invite|permission|role|policy|access|credential|apikey|token|secret/i)),
        }),
        policy({
            ...
            when: any(tagIncludes(secretTag), actionMatches(/secret|password|key|token|credential|vault|kms|sign/i), flag('payload.containsSecretMaterial')),
        }),
        policy({
            ...
            when: any(tagIncludes(externalCommTag), actionMatches(/email|send|post|publish|broadcast|notify|share|webhook/i), ...corporateDomains.length > 0 ? [externalDomainNotAllowed('payload.recipientEmail', corporateDomains)] : []),
        }),
        policy({
            ...
            when: any(actionMatches(/export|download|dump|backup|extract/i), recipientCountAbove(exportRecordThreshold, 'payload.recordCount')),
        }),
        policy({
...
export function createFutureProofPolicies(options = {}) {
